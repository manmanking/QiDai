...
//static const CGFloat kSectionHeaderMarginm = 25.f;
//static const CGFloat kSectionHeaderHeight = 22.f;
//static const CGFloat kTableCellHeight = 42.f;
...
- (void)viewDidLoad {
    ...
    //[self creatTitleViewWithString:@"离线地图"];
    [self creatTitleViewWithString:@"离线地图"];
    
    [self creatRightBtnWithString:@"全部取消"];
    [self setupCities];
...
- (NSString *)cellDetailTextForItem:(MAOfflineItem *)item
{
    ...
    {
        return [NSString stringWithFormat:@"大小:%.2fM", (float)item.size/1024.0/1024.0];
    }
    ...
    if (![self.downloadingItems containsObject:item])
    {
        ...
        {
            detailText = [NSString stringWithFormat:@"%.2f/%.2fM", item.downloadedSize/1024.0/1024.0, item.size/1024.0/1024.0];
        }
        else
        {
            detailText = [NSString stringWithFormat:@"大小:%.2fM", (float)item.size/1024.0/1024.0];
        }
    }
    else
    {
        ...
        switch (status)
        {
            ...
            case MAOfflineMapDownloadStatusProgress:
            {
                ...
                detailText = [NSString stringWithFormat:@"%.2f/%.2fM(%.1f%%)", recieved/1024.0/1024.0, expected/1024.0/1024.0, recieved/(float)expected*100];
